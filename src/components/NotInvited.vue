...
=======
  ...
  export default {
    ...
    mounted() {
      ...
      this.typeText(this.$refs.line1, "Se você não foi convidado para o lar dos fitas, o que veio fazer aqui?", () => {
        this.typeText(this.$refs.line2, "Só pode entrar se for convidado.", () => {
          setTimeout(() => {
            this.showOptions = true;
          }, 5000); // delay
    ...
    methods: {
      typeText(element, text, callback) {
        ...
        }, 100); // Ajuste o valor para alterar a velocidade de digitação
      },
      contactDanielle() {
        window.location.href = 'https://wa.me/5531997077639';
      },
      contactElias() {
        window.location.href = 'https://wa.me/5531973112693';
      },
      closePage() {
        this.showButtons = false;
        this.showMessage = true;
        ...
        setTimeout(() => {
          window.close();
          if (!window.closed) {
            alert('Se você desisti tão fácil assim realmente não é bem vindo kkkkkkk');
          }
        }, 100);
      }
    },
